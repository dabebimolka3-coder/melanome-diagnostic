...
/* Masquer les boutons Streamlit par défaut dans la topbar */
...
# ── TOPBAR AVEC BOUTONS STREAMLIT (STYLE ANALYSE | MÉTHODOLOGIE | DOCUMENTATION | CONTACT) ──
# Utilisation de colonnes pour organiser la topbar
...
with col2:
    ...
    with nav_cols[0]:
        if st.button("ANALYSE", key="nav_analyse", use_container_width=True):
            ...
            st.rerun()
    with nav_cols[1]:
        st.markdown('<p style="text-align: center; color: rgba(0,0,0,0.15); margin: 0;">|</p>', unsafe_allow_html=True)
    with nav_cols[2]:
